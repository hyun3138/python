...
# 파일 추가
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요.", filetypes=(("PNG 파일", "*.png"), ("모든 파일", "*.*")), initialdir="C:/") # 최초에 C:/ 경로를 보여줌 (사용자가 지정한 경로)
...
def merge_image():
    try: 
        ...
        # 계산식
        # 100 * 60 이미지 -> width 80으로 줄이면 height ?
        # (원본 width) : (원본 heigth) = (변경 width) : (변경 height)
        #     100            60              80             ?
        #      100*? = 80*60
        #    ? = 80*60/100 -> 이 식을 적용
